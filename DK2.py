...
def SOLVE(z0,*data):
  ...
  for k in range(0,2):
     ...
     omt = -omp*(kh_kt)
     '''
     D[0,0] += 2*ze[k]*(f.p(z0,2)*(1-1.5*omt)+f.p(z0,4)*omt)/rkd[k]
     D[0,1] += 2*ze[k]*(-vd*f.p(z0,1)*(1-1.5*omt)-vd*f.p(z0,3)*omt\
               +j*np.sin(phi)*kprho[k]/2.0*f.p(z0,1)*(1-0.5*omt)\
               +j*np.sin(phi)*kprho[k]/2.0*f.p(z0,3)*omt)/rkd[k]
     D[0,2] += 2*ze[k]*(-j*np.cos(phi)*kprho[k]/2.0*f.p(z0,1)*(1-0.5*omt)\
               -j*np.cos(phi)*kprho[k]/2.0*f.p(z0,3)*omt)/rkd[k]
     D[1,0] += 2*ze[k]*(-vd*f.p(z0,1)*(1-1.5*omt)-vd*f.p(z0,3)*omt\
               -j*np.sin(phi)*kprho[k]/2.0*f.p(z0,1)*(1-0.5*omt)\
               -j*np.sin(phi)*kprho[k]/2.0*f.p(z0,3)*omt)/rkd[k]
     D[1,1] += 2*ze[k]*(kxrho[k]**2/2.0*z0\
               +(vd**2+np.sin(phi)**2*kprho[k]**2/2.0)*f.p(z0,0))/rkd[k]
     D[1,2] += 2*ze[k]*(j*kxrho[k]/2.0*(1-0.5*omt)+j/4.0*kxrho[k]*omt\
               +j*np.cos(phi)*kprho[k]/2.0*vd*f.p(z0,0)-\
               kprho[k]**2/4.0*np.sin(2*phi)*f.p(z0,0))/rkd[k]
     D[2,0] += 2*ze[k]*(j*np.cos(phi)*kprho[k]/2.0*f.p(z0,1)*(1-0.5*omt)\
               +j*np.cos(phi)*kprho[k]/2.0*f.p(z0,3)*omt)/rkd[k]
     D[2,1] += 2*ze[k]*(-j*kxrho[k]/2.0*(1-0.5*omt)-j/4.0*kxrho[k]*omt\
               -j*np.cos(phi)*kprho[k]/2.0*vd*f.p(z0,0)-\
               kprho[k]**2/4.0*np.sin(2*phi)*f.p(z0,0))/rkd[k]
     ...
     D[1,1] += 2*ze[k]*(kxrho[k]**2/2.0*z0\
               +(vd**2+np.sin(phi)**2*kprho[k]**2/2.0)*f.p(z0,0))/rkd[k]
     #D[1,2] += 2*ze[k]*(j*kxrho[k]/2.0)/rkd[k]
     D[1,2] += 2*ze[k]*(j*kxrho[k]/2.0*(1-0.5*omt)+j/4.0*kxrho[k]*omt\
               +j*np.cos(phi)*kprho[k]/2.0*vd*f.p(z0,0)-\
               kprho[k]**2/4.0*np.sin(2*phi)*f.p(z0,0))/rkd[k]
     D[2,0] += 2*ze[k]*(j*np.cos(phi)*kprho[k]/2.0*f.p(z0,1))/rkd[k]
     #D[2,1] += 2*ze[k]*(-j*kxrho[k]/2.0)/rkd[k]
     D[2,1] += 2*ze[k]*(-j*kxrho[k]/2.0*(1-0.5*omt)-j/4.0*kxrho[k]*omt\
               -j*np.cos(phi)*kprho[k]/2.0*vd*f.p(z0,0)-\
               kprho[k]**2/4.0*np.sin(2*phi)*f.p(z0,0))/rkd[k]
     D[2,2] += 2*ze[k]*(np.cos(phi)**2*kprho[k]**2/2.0*f.p(z0,0)+\
