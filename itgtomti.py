...
for j in range(0,num):
   ...
   for i in range(0,num):
    ...
    sol = root(NGKD.SOLVE4,(zeta.real,zeta.imag),args=data,method='hybr',tol=1e-5)
    fzeta[j,i] = complex(sol.x[0],sol.x[1])

    fzeta1[j,i] = fzeta[j,i]*kxrho[j]/rho_H
    if fzeta[j,i].imag<0:
      fzeta1[j,i] = complex(0,1e-10)
    print fzeta1[j,i]
...
myplt = plt.imshow(Z,extent= extents,cmap=cmap,
      aspect='auto',origin='lower',vmin=0.0,vmax=1.4)
#myplt = plt.imshow(Z,extent= extents,cmap=cmap,
...
#plt.savefig('itg0.eps',format='eps',dpi=300,bbox_inches='tight')
plt.show()
