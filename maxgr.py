...
for j in range(0,num):
  ...
  for i in range(0,num):
    ...
    if fzeta[num-j-1,i].imag>maxgrowth and fzeta[num-j-1,i].imag<20.0 and kprho[i]>1.0 and kprho[i]<60.0:
      maxgrowth = fzeta[num-j-1,i].imag
      maxkxrho = kxrho[j]
      maxkprho = kprho[i]
    if fzeta[num-j-1,i].imag>maxgrowth1 and fzeta[num-j-1,i].imag<5.0 and kprho[i]<1.0:
      if kxrho[j]/rho_H>0.1:
        maxgrowth1 = fzeta[num-j-1,i].imag
        maxkxrho1 = kxrho[j]
        maxkprho1 = kprho[i]
    print 'fz',fzeta[num-j-1,i],kprho[i]

kxrho  = np.logspace(-3,-1,num)
for j in range(0,num):
  print( 'j=',j)
  for i in range(0,num):
    if i==0 and j==0:
    ...
    if fzeta[num+j,i].imag>maxgrowth and fzeta[num+j,i].imag<20. and kprho[i]>1.0 and kprho[i]<60.0:
      maxgrowth = fzeta[num+j,i].imag
      maxkxrho = kxrho[j]
      maxkprho = kprho[i]
    if fzeta[num+j,i].imag>maxgrowth1 and fzeta[num+j,i].imag<5.0 and kprho[i]<1.0:
      if kxrho[j]/rho_H>0.1:
        maxgrowth1 = fzeta[num+1,i].imag
        maxkxrho1 = kxrho[j]
        maxkprho1 = kprho[i]
...
######################################################################
######################## Plot the results #############################
extents = (1e-5/rho_H,1e-1/rho_H,kprho[0],kprho[-1])
...
print 'maxgrowh=',maxgrowth*np.sqrt(3.0),'maxkxrho=',maxkxrho,'maxkprho=',maxkprho
print 'maxgrowh=',maxgrowth1*np.sqrt(3.0),'maxkxrho=',maxkxrho1,'maxkprho=',maxkprho1
myplt = plt.imshow(Z,extent= extents,cmap=cmap,
      aspect='auto',origin='lower',vmin=0.0,vmax=2.0)
plt.xscale('log')
plt.yscale('log')
